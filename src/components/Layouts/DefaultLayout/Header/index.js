...
import { faEllipsisVertical, faCircleQuestion, faKeyboard, faCloudArrowUp, faPaperPlane, faMessage, faUser, faGear, faRightFromBracket, faCoins, faEarthAsia } from '@fortawesome/free-solid-svg-icons';
...
const menu_items_accessSuccessfully = [
    {
    ...
    ...menu_items,
    {
        ...
        title: 'Log out',
        separate: true //vach chia
    },
]

function Header() {
    const accessSuccessfully = true
        ...
        <header className={cx('wrapper')}>
            <div className={cx('container')}>
                ...
                <div className={cx('actions')}>
                    {accessSuccessfully ? (
                        <>
                            <TippyNoHeadless content='Upload Video'>
                                <button className={cx('action-btn')}><FontAwesomeIcon icon={faCloudArrowUp} /></button>
                            </TippyNoHeadless>
                            <TippyNoHeadless content='Message'>
                                <button className={cx('action-btn')}><FontAwesomeIcon icon={faPaperPlane} /></button>
                            </TippyNoHeadless>
                            <TippyNoHeadless content='Inbox'>
                                <button className={cx('action-btn')}><FontAwesomeIcon icon={faMessage} /></button>
                            </TippyNoHeadless>
                        </>
                    ) : (
                        <>
                            <Button btnUpload>Upload</Button>
                            <Button btnLoginHeader>Login</Button>

                        </>
                    )}
                    <MenuItem items={accessSuccessfully ? menu_items_accessSuccessfully : menu_items} onChange={menuItem => { console.log(menuItem) }}>
                        {accessSuccessfully ? (
                            <button className={cx('action-btn')}><img src='https://htran844.github.io/hihi/img/ig/i3.jpg' alt='avatar' onError={e => {
